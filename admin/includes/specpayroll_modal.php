...
<!-- <script>
    $(document).ready(function() {
        $('.payroll').click(function fetchDataFromTable(payroll) {
            var id = $(this).data('id');
            var RecId = $(this).closest('tr').find('td:eq(1)').text(); // Extract RecId from table
            var Employee_Name = $(this).closest('tr').find('td:eq(2)').text(); // Extract Employee_Name from table
            var designation_name = $(this).closest('tr').find('td:eq(5)').text(); // Extract designation_name from table
            var shift_name = $(this).closest('tr').find('td:eq(7)').text(); // Extract shift_name from table
            var pay_name = $(this).closest('tr').find('td:eq(6)').text(); // Extract pay_name from table
            var time_in = $(this).closest('tr').find('td:eq()').text(); // Extract time_in from table
           
            var time_out = $(this).closest('tr').find('td:eq()').text(); // Extract time_out from table
            var salary = $(this).closest('tr').find('td:eq(11)').text(); // Extract salary from table
            var deducted_days = $(this).closest('tr').find('td:eq()').text(); // Extract deducted_days from table
            var late = $(this).closest('tr').find('td:eq()').text(); // Extract late from table
            var absent = $(this).closest('tr').find('td:eq()').text(); // Extract absent from table
            var Advance = $(this).closest('tr').find('td:eq(12)').text(); // Extract advance from table
            var Deduction = $(this).closest('tr').find('td:eq()').text(); // Extract Deduction from table
            var M_Deducted = $(this).closest('tr').find('td:eq()').text(); // Extract M_Deducted from table
            var M_Salary = $(this).closest('tr').find('td:eq()').text(); // Extract M_Salary from table
            var Total_Pay = $(this).closest('tr').find('td:eq()').text(); // Extract M_Total_Pay from table
            $('#posid').val(id);
